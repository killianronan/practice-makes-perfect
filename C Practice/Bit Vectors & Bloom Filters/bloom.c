...
int bloom_lookup(struct bloom * this, char * item) {
    int index = hash_string(item,BLOOM_HASH1,this->size);
    int index2 = hash_string(item,BLOOM_HASH2,this->size);
    if(bitset_lookup(this->bitset,index)==1){
        if(bitset_lookup(this->bitset,index2)==1){
            return 1;
        }
    }
    return 0;
