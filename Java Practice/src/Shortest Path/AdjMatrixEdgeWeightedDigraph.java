...
public class AdjMatrixEdgeWeightedDigraph {
    ...
    public AdjMatrixEdgeWeightedDigraph(Scanner scanner){
        if (scanner == null) throw new IllegalArgumentException("argument is null");
        this.V = scanner.nextInt();
        if (V < 0) throw new IllegalArgumentException("number of vertices in a Digraph must be nonnegative");
        if(this.V < 3)this.isValidGraph = false;
        ...
        if(this.isValidGraph){
            ...
            for (int i = 0; i < edges; i++) {
                ...
                if( j >= 0 && k>= 0)
                    if (k==j) 
                        addEdge(new DirectedEdge(j, k, Math.abs(weight)));
                    else 
                    	addEdge(new DirectedEdge(j, k, weight));
                else
