...
someFunc = do
  ...
  putStrLn $ "my github account is " ++ user
  putStrLn $ "my generated token is " ++ token
...
testGitHubCall auth name = 
  (SC.runClientM (GH.getUser (Just "github-access-haskell") auth name) =<< env) >>= \case
    ...
    Right res -> do
      ...
      (SC.runClientM (GH.getUserRepos (Just "github-access-haskell") auth name) =<< env) >>= \case
        Left err -> do
          putStrLn $ "Error encountered while retrieving repo data: " ++ show err
        Right repos -> do
          ...
          partitionEithers <$> mapM (getContribs auth name) repos >>= \case
            ...
            (ers, _)-> do
              putStrLn $ "Error encountered while retrieving contributors: " ++ show ers
